...
### Phase 7: 游戏流程（预计 1-2 周）

> 将移植好的 Lua 框架与 Unity 游戏流程串联

#### 7.1 流程状态机
- [ ] Flow.lua 状态机集成
- [ ] 状态：Init → Lobby → SelectHero → SelectDungeon → Loading → InDungeon → Settlement
- [ ] 验证：状态切换正常

#### 7.2 UI 管理
- [ ] UIManager.cs - 基于 GameState 控制面板显示
- [ ] 基础面板：LobbyPanel、SelectHeroPanel、SelectDungeonPanel、BattleHUD、SettlementPanel
- [ ] 验证：面板随状态自动切换

#### 7.3 实体可视化
- [ ] EntityService.cs - 为 Lua 单位创建 Unity GameObject
- [ ] 临时使用 Primitive 形状（Capsule 表示单位）
- [ ] 验证：Lua 创建单位时 Unity 同步显示
...
#### 8.1 副本管理
- [ ] Dungeon.lua - 副本初始化、状态管理
- [ ] DungeonRoom.lua - 房间管理、刷怪脚本
- [ ] 验证：副本能创建并进入
...
### Phase 9: 战斗系统集成（预计 1 周）

> 将技能系统、伤害系统、Buff 系统集成到游戏流程
...
#### 9.2 伤害结算
- [ ] Fight:ApplyDirectlyDamage 集成
- [ ] HP 减少 → 死亡检测 → Kill()
- [ ] 验证：伤害正确扣血，死亡正确处理

#### 9.3 Buff 系统
- [ ] SkillAPI.AddEntityBuff / RemoveEntityBuff
- [ ] Buff 效果生效（属性修改）
- [ ] 验证：Buff 添加/移除/过期正常

### Phase 10: 战斗 UI 表现（预计 1 周）
...
### 6.9 Phase 9 验收
- [ ] 技能释放流程正常（SkillAPI.DoSkill）
- [ ] 伤害结算正确（HP 减少、死亡处理）
- [ ] Buff 系统正常（添加/移除/效果生效）
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v0.1 | 2024-12-31 | 创建文档，完成整体规划 (Phase 1-6) |
| v0.2 | 2024-12-31 | 补充 Phase 7-10 设计规划及验收标准 |
